...
def relation(pedestrian_boxes, face_boxes):
    ...
    p_cx = (p_x2 + p_x1) * 0.5
    p_cy = (p_y2 + p_y1) * 0.5
    p_x1, p_y1, p_x2, p_y2 = p_cx - p_w, p_cy - p_h, p_cx + p_w, p_cy + p_h
    idx = []
    for i in range(len(face_boxes)):
        is_in = (f_scores[i] >= 0.5) | ((p_x1 <= f_x1[i]) & (p_y1 <= f_y1[i]) & (p_x2 >= f_x2[i]) & (p_y2 >= f_y2[i])).any()
        if is_in:
            idx.append(i)
...
def pred_eval(predictor, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    ...
    not_cnt = 0
    for idx_im in range(0, num_images):
        all_boxes[2][idx_im], changed = relation(all_boxes[1][idx_im], all_boxes[2][idx_im], cfg.TEST.SOFTNMS_THRESH)
        if not changed:
            not_cnt += 1
    print('Total: {}, Not: {}'.format(num_images, not_cnt))
    '''
